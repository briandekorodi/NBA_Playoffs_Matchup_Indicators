...
for (year in 2002:2024) {
  ...
  tryCatch({
    ...
    df <- df[, !colnames(df) %in% c("oppLayups%FGA", "oppMade_Layups", "Col33")]
...
### Historical Probability of winning a series based on Net Rating

```{r, message=FALSE, warning=FALSE}
net_rating_summary <- do.call(rbind, Playoffs_FourFactors) %>%
  filter(`Series_Abs_Net_Rating` != 0) %>%
  mutate(
    Net_Rating_Interval = cut(
      `Series_Abs_Net_Rating`,
      breaks = seq(0, max(`Series_Abs_Net_Rating`, na.rm = TRUE) + 1, by = 1),
      right = FALSE,
      include.lowest = TRUE
    )
  ) %>%
  group_by(Net_Rating_Interval) %>%
  summarise(
    count = n() / 2,
    win_probability = sum(`Series_Net_Rating` > 0 & `Series_Win/Loss` == 1) / (n() / 2),
    loss_probability = sum(`Series_Net_Rating` < 0 & `Series_Win/Loss` == 1) / (n() / 2)
  ) %>%
  arrange(Net_Rating_Interval)

ggplot(net_rating_summary, aes(x = Net_Rating_Interval, y = win_probability)) +
  geom_point(color = "blue", size = 3) +
  geom_line(group = 1, color = "blue", size = 1) +
  labs(
    title = "Historical Win Probability by Net Rating Interval",
    x = "Net Rating",
    y = "Win Probability"
  ) +
...
### Probability of Margins of Victory based on Net rating

The aim being to create betting strategies, based on a prediction of the net rating of a playoff matchup, for

1)  Which team will win the series,

2)  In how many games the winner will clinch the series,

we must convert our net rating predictions into probabilities of victory and margins of victory. For this I suggest we use a historical probability method, where we observe, over our sample of 420 playoff series, the likelihood of each series outcome and victory margin for each net rating.

```{r, message=FALSE, warning=FALSE}
net_rating_margin_victory <- do.call(rbind, Playoffs_FourFactors) %>%
  filter(`Series_Abs_Net_Rating` != 0) %>%
  mutate(
    Net_Rating_Interval = cut(
      `Series_Abs_Net_Rating`,
  ...
  summarise(
    count = n() / 2,
    probability_of_winning_in_7 = sum(`Series_Net_Rating` > 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 1) / (n() / 2),
    probability_of_winning_in_6 = sum(`Series_Net_Rating` > 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 2) / (n() / 2),
    probability_of_winning_in_5 = sum(`Series_Net_Rating` > 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 3) / (n() / 2),
    probability_of_winning_in_4 = sum(`Series_Net_Rating` > 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 4) / (n() / 2),
    probability_of_losing_in_7 = sum(`Series_Net_Rating` < 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 1) / (n() / 2),
    probability_of_losing_in_6 = sum(`Series_Net_Rating` < 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 2) / (n() / 2),
    probability_of_losing_in_5 = sum(`Series_Net_Rating` < 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 3) / (n() / 2),
    probability_of_losing_in_4 = sum(`Series_Net_Rating` < 0 & `Series_Win/Loss` == 1 & `Series_Victory_Margin` == 4) / (n() / 2)
  ) %>%
  arrange(Net_Rating_Interval)

net_rating_margin_victory_long <- net_rating_margin_victory %>%
  select(Net_Rating_Interval, 
         probability_of_winning_in_7, 
         probability_of_winning_in_6, 
         probability_of_winning_in_5, 
         probability_of_winning_in_4, 
         probability_of_losing_in_7, 
         probability_of_losing_in_6, 
         probability_of_losing_in_5, 
         probability_of_losing_in_4) %>%
  pivot_longer(cols = -Net_Rating_Interval, 
               names_to = "Outcome", 
               values_to = "Probability")

ggplot(net_rating_margin_victory_long, aes(x = Net_Rating_Interval, y = Probability, color = Outcome, group = Outcome)) +
  geom_line(size = 1) +
  geom_point(size = 1) +
  labs(
    title = "Probabilities of Winning/Losing in Different Series Lengths by Net Rating Interval",
    x = "Net Rating Interval",
    y = "Probability",
    color = "Outcome"
  ) +
  theme_minimal() +
...
#### Historical Average Victory Margin by Net Rating

We start in 2004 because that is when the first round became best-of-seven series.

```{r, message=FALSE, warning=FALSE}
plot_data <- data.frame(Series_Net_Rating = numeric(0), Series_Victory_Margin = numeric(0), Year = integer(0), Series = character(0))
for (year in 2004:2024) {
  for (file in Playoff_series) {
    df <- Playoffs_FourFactors[[paste0(file, "_", year)]]
    
    if (!is.null(df) && nrow(df) > 1) {
      net_rating_value <- df$`Series_Net_Rating`[1]
      victory_margin_value <- df$`Series_Victory_Margin`[1]
      plot_data <- rbind(plot_data, data.frame(Series_Net_Rating = net_rating_value, 
                                               Series_Victory_Margin = victory_margin_value, 
                                               Year = year, 
                                               Series = file))
    }
  }
...
plot_data <- plot_data %>%
  mutate(Net_Rating_Binned = cut(Series_Net_Rating, 
                                 breaks = seq(floor(min(Series_Net_Rating)), ceiling(max(Series_Net_Rating)), by = 1), 
                                 include.lowest = TRUE, right = FALSE, 
                                 labels = seq(floor(min(Series_Net_Rating)), ceiling(max(Series_Net_Rating)) - 1, by = 1)))
avg_victory_margin <- plot_data %>%
  group_by(Net_Rating_Binned) %>%
  summarize(mean_victory_margin = mean(Series_Victory_Margin, na.rm = TRUE))

ggplot(avg_victory_margin, aes(x = as.numeric(as.character(Net_Rating_Binned)), y = mean_victory_margin)) +
  geom_line(color = "blue") +       
  geom_point(color = "red") +   
  geom_smooth() +
  theme_minimal() +                   
  labs(title = "Historical Average Victory Margin by Net Rating (1.0 Intervals)",
       x = "Net Rating",
       y = "Average Victory Margin") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

#### Historical Probability of each Victory Margin by Net rating

```{r, message=FALSE, warning=FALSE}
plot_data <- plot_data %>%
  mutate(Net_Rating_interval = cut(Series_Net_Rating, breaks = seq(floor(min(Series_Net_Rating)), ceiling(max(Series_Net_Rating)), by = 1), 
                                   include.lowest = TRUE, right = FALSE))

probability_data <- plot_data %>%
  group_by(Net_Rating_interval, Series_Victory_Margin) %>%
  tally() %>%
  ungroup()

total_counts <- probability_data %>%
  group_by(Net_Rating_interval) %>%
  summarize(total_count = sum(n))

probability_data <- probability_data %>%
  left_join(total_counts, by = "Net_Rating_interval") %>%
  mutate(probability = n / total_count)

ggplot(probability_data, aes(x = Net_Rating_interval, y = probability, color = as.factor(Series_Victory_Margin))) +          
  geom_point() +
  geom_line(aes(group = Series_Victory_Margin)) +
  theme_minimal() +                         
  labs(title = "Historical probability of Victory Margin by Net Rating (1.0 intervals)",
       x = "Net Rating Interval",
       y = "Probability",
       color = "Victory Margin") +
  scale_color_manual(values = c("blue", "green", "yellow", "red")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

#### Histogram of Net Rating

To show statistical strength of samples predicting the victory margin based on net rating. Both extremities have very few samples, hence the nonsensical variability. We have a solid number of cases of Net Ratings in the range of about -1 to +12 for winners, it is fairly informative. Outside that range we just know that the probability of 1 game and 4 game margins are extremely high, but the variability is just randomness.

```{r, message=FALSE, warning=FALSE}
ggplot(plot_data, aes(x = Series_Net_Rating)) + 
  geom_histogram(binwidth = 1, fill = "skyblue", color = "black", alpha = 0.7) + 
  theme_minimal() +
  labs(title = "Histogram of Net Rating (1.0 intervals)",
       x = "Net Rating",
       y = "Frequency") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r, message=FALSE, warning=FALSE}
for (name in names(Playoffs_FourFactors)) {
  year <- as.integer(str_extract(name, "\\d{4}"))
  season_name <- paste0("Season_", year)
  
  if (season_name %in% names(Seasons)) {
    Playoffs_FourFactors[[name]] <- merge(
      Playoffs_FourFactors[[name]], 
      Seasons[[season_name]], 
      by = "Team", 
      all.x = TRUE
    )}
