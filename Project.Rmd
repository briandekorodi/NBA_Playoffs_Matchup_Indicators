...
```{r}
team_stats <- c("FGMr", "FGAr", "FG%", "3PMr", "3PAr", "3P%", "2PMr", "2PAr", "2P%", "FTAr", "ORBr", "DRBr", "TRBr", "ASTr", "STLr", "BLKr", "TOVr", "PFr", "ORtg", "Average_age", "Wins", "Expected_wins", "MOV", "SRS", "NRtg", "Pace", "eFG%", "TOV%", "ORB%", "FT_per_FGA", "Attendance_per_game", "Average_FGA_distance", "2P%FGA", "0_3%FGA", "3_10%FGA",       "10_16%FGA", "16_3P%FGA", "3P%FGA", "0_3FG%", "3_10FG%", "10_16FG%", "16_3PFG%",            "2P%FGAst", "3P%FGAst", "Dunks%FGA", "Made_dunks", "Corner%3PA", "Corner3P%")
opponent_stats <- c("oppFGMr", "oppFGAr", "oppFG%", "opp3PMr", "opp3PAr", "opp3P%", "opp2PMr", "opp2PAr", "opp2P%", "oppFTAr", "oppORBr", "oppDRBr", "oppTRBr", "oppASTr", "oppSTLr", "oppBLKr", "oppTOVr", "oppPFr", "DRtg", "Average_age", "Wins", "Expected_wins", "MOV", "SRS", "NRtg", "Pace", "oppeFG%", "oppTOV%", "DRB%", "oppFT_per_FGA", "Attendance_per_game", "oppAverage_FGA_distance", "opp2P%FGA", "opp0_3%FGA", "opp3_10%FGA", "opp10_16%FGA", "opp16_3P%FGA", "opp3P%FGA", "opp0_3FG%", "opp3_10FG%", "opp10_16FG%", "opp16_3PFG%",      "opp2P%FGAst", "opp3P%FGAst", "oppDunks%FGA", "oppMade_dunks", "oppCorner%3PA", "oppCorner3P%")
...
for (name in names(Playoffs)) {
  if (!is.null(Playoffs[[name]]) && nrow(Playoffs[[name]]) == 2) {
    ...
    df$`W_Team_O_Diff_0_3FG%` <- c(
        df[["Diff_0_3FG%"]][1] * df[["0_3%FGA"]][1],
        df[["Diff_0_3FG%"]][2] * df[["0_3%FGA"]][2]
      )
    df$`W_Opp_D_Diff_0_3FG%` <- c(
        df[["Diff_0_3FG%"]][1] * df[["opp0_3%FGA"]][2],
        df[["Diff_0_3FG%"]][2] * df[["opp0_3%FGA"]][1]
      )
    df$`W_Team_O_Diff_3_10FG%` <- c(
        df[["Diff_3_10FG%"]][1] * df[["3_10%FGA"]][1],
        df[["Diff_3_10FG%"]][2] * df[["3_10%FGA"]][2]
      )
    df$`W_Opp_D_Diff_3_10FG%` <- c(
        df[["Diff_3_10FG%"]][1] * df[["opp3_10%FGA"]][2],
        df[["Diff_3_10FG%"]][2] * df[["opp3_10%FGA"]][1]
      )
    df$`W_Team_O_Diff_10_16FG%` <- c(
        df[["Diff_10_16FG%"]][1] * df[["10_16%FGA"]][1],
        df[["Diff_10_16FG%"]][2] * df[["10_16%FGA"]][2]
      )
    df$`W_Opp_D_Diff_10_16FG%` <- c(
        df[["Diff_10_16FG%"]][1] * df[["opp10_16%FGA"]][2],
        df[["Diff_10_16FG%"]][2] * df[["opp10_16%FGA"]][1]
      )
    df$`W_Team_O_Diff_16_3PFG%` <- c(
        df[["Diff_16_3PFG%"]][1] * df[["16_3P%FGA"]][1],
        df[["Diff_16_3PFG%"]][2] * df[["16_3P%FGA"]][2]
      )
    df$`W_Opp_D_Diff_16_3PFG%` <- c(
        df[["Diff_16_3PFG%"]][1] * df[["opp16_3P%FGA"]][2],
        df[["Diff_16_3PFG%"]][2] * df[["opp16_3P%FGA"]][1]
      )
    df$`W_Team_O_Diff_3P%` <- c(
        df[["Diff_3P%"]][1] * df[["3P%FGA"]][1],
        df[["Diff_3P%"]][2] * df[["3P%FGA"]][2]
      )
    df$`W_Opp_D_Diff_3P%` <- c(
        df[["Diff_3P%"]][1] * df[["opp3P%FGA"]][2],
        df[["Diff_3P%"]][2] * df[["opp3P%FGA"]][1]
      )
    Playoffs[[name]] <- df
  }
}
