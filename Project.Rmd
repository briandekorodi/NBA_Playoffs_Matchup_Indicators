...
```{r}
matchup_stats <- c("Games_Played", "Minutes_Played", "FGMr", "FGAr", "FG%", "3PMr", "3PAr", "3P%", "2PMr", "2PAr", "2P%", "FTMr", "FTAr", "FT%", "ORBr", "DRBr", "TRBr", "ASTr", "STLr", "BLKr", "TOVr", "PFr", "ORtg", "oppFGMr", "oppFGAr", "oppFG%", "opp3PMr", "opp3PAr", "opp3P%", "opp2PMr", "opp2PAr", "opp2P%", "oppFTMr", "oppFTAr", "oppFT%", "oppORBr",   "oppDRBr", "oppTRBr", "oppASTr", "oppSTLr", "oppBLKr", "oppTOVr", "oppPFr", "DRtg", "Average_age", "Wins", "Losses", "Expected_wins", "Expected_Losses", "MOV", "SOS", "SRS", "NRtg", "Pace", "FTA_per_FGA", "%3PA", "TS%", "eFG%", "TOV%", "ORB%", "FT_per_FGA", "oppeFG%", "oppTOV%", "DRB%", "oppFT_per_FGA", "Arena", "Attendance", "Attendance_per_Game", "Average_FGA_distance", "2P%FGA", "0_3%FGA", "3_10%FGA", "10_16%FGA", "16_3P%FGA", "3P%FGA", "0_3FG%", "3_10FG%", "10_16FG%", "16_3PFG%", "2P%FGAst", "3P%FGAst", "Dunks%FGA", "Made_dunks", "Corner%3PA", "Corner3P%", "Attempted_heaves", "Made_heaves", "oppAverage_FGA_distance", "opp2P%FGA",     "opp0_3%FGA", "opp3_10%FGA", "opp10_16%FGA", "opp16_3P%FGA", "opp3P%FGA", "opp0_3FG%", "opp3_10FG%", "opp10_16FG%", "opp16_3PFG%", "opp2P%FGAst", "opp3P%FGAst", "oppDunks%FGA", "oppMade_dunks", "oppCorner%3PA", "oppCorner3P%")

Playoffs_matchup <- list()

for (name in names(Playoffs)) {
    
    df <- Playoffs[[name]]
    
    for (i in seq_along(matchup_stats)) {
      team_col <- matchup_stats[i]
      matchup_col <- paste0("matchup_", team_col)
      
      if (team_col %in% names(df)) {
        df[[matchup_col]] <- c(df[[team_col]][2], df[[team_col]][1])
      }
    }
    Playoffs_matchup[[name]] <- df
  }
...
```{r}
Playoffs_combined <- do.call(rbind, Playoffs_matchup) %>%
  mutate(
...
Playoffs_combined_lm <- Playoffs_combined %>%
  select(-c(Result, Series_Pace, Series_eFGpct, Series_ORBpct, Series_TOVpct, Series_FTperFGA, Series_Abs_Net_Rating, Series_PTSperG, Series_Victory_Margin, Series_Net_Rating, `Series_Win/Loss`, Series_Abs_Net_Rating, Losses, Series, Arena, Team, Predicted_ORtg_rf, Series_Games))  %>%
  mutate(across(everything(), ~ replace_na(., 0)))
...
rmse_lm <- sqrt(mean((Playoffs_combined$Series_ORtg - Playoffs_combined$Predicted_ORtg_lm)^2))
cat("Root Mean Squared Error:", rmse_lm, "\n")

```

Factoring the matchup data increases the R-Squared to 0.65.

Considering we are not taking injuries into account, a R-Squared of 0.65 with very high significance for our model seems pretty strong. We have also extracted a list of factors that provide interesting interpretations. An expansion of the Four Factors idea to include the transition from Regular Season to Playoffs.

```{r}
Predictions <- Playoffs_combined %>%
  select(Team, Year, Series, Result, Series_ORtg, Predicted_ORtg_rf, Predicted_ORtg_lm)
```
